...
class ApiAction():
  ...
  def getVariables(self, yamlFile):
    ...
    self.json_file = info["vnfInfo"]["test_file_path"] + "/" + info["vnfInfo"]["json_file"]
    ...
    self.vnf_type = info["vnfInfo"]["vnf_type"]
    self.testId = info["vnfd"]["testId"]
    self.node_type_value = info["vnfInfo"]["node_type_value"]
  ...
  def writeVnfdId(self): 
    self.authTest() 
    if(self.cbam_version == "19.5" or self.cbam_version == "21" ):
    ...
    if self.inputVnfdId in usedVnfdIds:
      ...
      vnfd["dsl_definitions"]["descriptor_id"] = self.inputVnfdId
      vnfd["dsl_definitions"]["product_name"] = self.inputVnfdId
      vnfd["dsl_definitions"]["product_info_name"] = self.inputVnfdId
      vnfd["node_types"][self.node_type_value]["properties"]["descriptor_id"]["constraints"] = [{'equal': self.inputVnfdId}]
      vnfd["node_types"][self.node_type_value]["properties"]["descriptor_id"]["default"] = self.inputVnfdId
  ...
  def uploadZipToCatalog(self):
    ...
    if(self.cbam_version == "19.5" or self.cbam_version == "21" ):
      ...
      print json.loads(vnfpckgid.text)
  ...
  def createVnf(self):
    self.authTest()
    body = {"vnfdId": self.vnfdId, "vnfInstanceName": self.testId, "vnfInstanceDescription": "testingApis"}
  ...
  def checkLcmStatus(self, vnfInstanceId, raise_error):
    self.authTest()
    param_holder =  {'vnfInstanceId' : vnfInstanceId }
    lcmStatus = requests.get(self.ip+'/vnflcm/v1/vnf_lcm_op_occs', verify=False, headers=self.auth_header, params=param_holder)
    status = json.loads(lcmStatus.text)[0]["operationState"]
    print "status: " + status
    if status == "FAILED"  or status == "FAILED_TEMP":
      if (raise_error):
        self.setFailState(json.loads(lcmStatus.text)[0]["id"])
        raise ValueError(json.loads(lcmStatus.text)[0]["operationState"])
      else:
        print("return Failed")
        self.setFailState(json.loads(lcmStatus.text)[0]["id"])
        return ("FAILED", json.loads(lcmStatus.text)[0]["operationState"])
    elif status == "COMPLETED":
        ##before cbam19sp2 is FINISHED
      return ("FINISHED", status)
    else:
        ##actual status is PROCESSING
      print "in progress"
  ...
  def checkInstantiationStatus(self):
    self.authTest()
    param_holder =  {'vnfInstanceId' : vnfInstanceId }
    instantiationStatus = requests.get(self.ip+'/vnflcm/v1/vnf_instances/', verify=False, headers=self.auth_header, params=param_holder)
  ...
  def setFailState(self, operationId):
    self.authTest()
    r = requests.post( self.ip + '/vnflcm/v1/vnf_lcm_op_occs/'+ operationId +'/fail', verify=False, headers=self.auth_header )
  ...
  def pollLcmStatus(self, raise_error):
    ...
    while(finished is False):
      ...
      if operationsText[0] != "FINISHED" and operationsText[0] != "FAILED":
        ...
        finished = True
    operationsState = operationsText[1]
    return operationsState
    

  def patchHaAction(self, direction):
    self.authTest()
    header = {'Authorization': (self.auth_header)['Authorization'], 'Content-Type': 'application/json'}
    if (direction == "false"):
        haRequest = {"vnfConfigurableProperties":{  "operation_triggers": { "auto_ha": { "enabled": False }}}}
    else:
        haRequest = {"vnfConfigurableProperties":{  "operation_triggers": { "auto_ha": { "enabled": True }}}}

    haAction = requests.patch(self.ip+'/vnflcm/v1/vnf_instances/'+self.vnfInstanceId, verify=False, headers=header, data=json.dumps(haRequest))
  ...
  def scaleAspectStatus(self, aspectId):
    self.authTest()
    aspect_status = requests.get(self.ip + '/vnflcm/v1/vnf_instances/' + self.vnfInstanceId, verify=False, headers=self.auth_header)
    json_body = json.loads(aspect_status.text)["instantiatedVnfInfo"]["scaleStatus"]
    for x in json_body:
      if x["aspectId"] == aspectId:
        scale_level = x["scaleLevel"]
  ...
  def healVnf(self,heal_type): 
    self.authTest()
    if( self.product_type == "generic" ):
       healData={'cause': 'testing', 'additionalParams': { 'heal_processingON': False, 'heal_crdb': False, 'heal_cgf': False, 'heal_ui': False, 'heal_db': False, 'heal_oam': False, 'heal_processingOFF': False }}
    if( self.product_type == "epc" ):
    ...
    healData['additionalParams'][heal_type] = True
  ...
  def updatePasswordLCM(self, recv_data):
    ...
    action = requests.post(self.ip + '/vnflcm/v1/vnf_instances/' + self.vnfInstanceId + '/custom/update_passwords', verify=False, headers=self.auth_header, data=json.dumps(custom_request))
    print(action.text)
  ...
  def haLCM(self):
    ...
    custom_request = {"additionalParams": {}}
    print(json.dumps(custom_request))
    action = requests.post(self.ip + '/vnflcm/v1/vnf_instances/' + self.vnfInstanceId + '/custom/ha', verify=False, headers=self.auth_header, data=json.dumps(custom_request))
    print(action.text)
  ...
  def rpmUpdateLCM(self, recv_data):
    ...
    for key in recv_data:
      ...
      custom_request["additionalParams"][key] = recv_data[key]
    print(json.dumps(custom_request))
    action = requests.post(self.ip + '/vnflcm/v1/vnf_instances/' + self.vnfInstanceId + '/custom/rpm_update', verify=False, headers=self.auth_header, data=json.dumps(custom_request))
    print(action.text)
  ...
  def customLcmAction(self, recv_data):
    ...
    print(json.dumps(custom_request))
    action = requests.post(self.ip + '/vnflcm/v1/vnf_instances/' + self.vnfInstanceId + '/custom/run_custom_ansible', verify=False, headers=self.auth_header, data=json.dumps(custom_request))
    print(action.text)

  def modify(self, recv_data):
    ##no idea on the usage this, this is original code before cbam21
    self.authTest()
  ...
  def getVnfInfo(self, yamlPath):
    # this is original code before cbam21, usage on smoke stream which we calling vertex side
    self.authTest()
  ...
  def deleteVnf(self):
    deleteInstance = requests.delete(self.ip+'/vnflcm/v1/vnf_instances/'+self.vnfInstanceId, headers=self.auth_header,verify=False)
  ...
  def checkPackageBeforeUpload(self, descriptor_id, productName):
    self.authTest()
    catalogList = requests.get(self.ip +'/vnfpkgm/v1/vnf_packages', verify=False, headers=self.auth_header)
    cbamCatalog = (json.loads(catalogList.text))

    for n in cbamCatalog:
      try:
        if n['vnfdId'] == descriptor_id or n['vnfProductName'] == productName:
          if not bool(n['userDefinedData']['references']):
            catalogDelete = requests.delete(self.ip+'/vnfpkgm/v1/vnf_packages/' + n['id'], headers=self.auth_header, verify=False)
          else:
            print "Below package is using same productor or description id in cbam"
            print n['userDefinedData']['references']
